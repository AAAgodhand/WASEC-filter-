...
.tools {{ display:flex; flex-wrap:wrap; gap:8px; align-items:center; background:#fff; border:1px solid var(--line); border-radius:8px; padding:11px 14px; margin:0 0 14px; }}
.tools input {{ flex:1; min-width:220px; padding:7px 11px; border:1px solid #cfd5df; border-radius:6px; font-family:inherit; font-size:13px; }}
.tools input:focus {{ outline:none; border-color:var(--navy); }}
.chip {{ padding:6px 12px; border:1px solid #cfd5df; background:#fff; border-radius:14px; font-size:12px; font-weight:bold; color:#44506b; cursor:pointer; font-family:inherit; }}
.chip:hover {{ background:#eef1f6; }}
.chip.on {{ background:var(--navy); color:#fff; border-color:var(--navy); }}
.count {{ font-size:12px; color:#666; margin-left:auto; white-space:nowrap; }}
tr.hide {{ display:none; }}
.empty {{ padding:16px; text-align:center; color:#888; font-size:13px; background:#fff; border:1px solid var(--line); border-top:none; }}
...
<div class="tools">
<input id="q" type="search" placeholder="Search title, agency, industry or finding…" autocomplete="off">
<button class="chip on" data-f="all">All</button>
<button class="chip" data-f="verified">Verified only</button>
<button class="chip" data-f="soon">Closing soon</button>
<button class="chip" data-f="icn">ICN login-wall</button>
...
(function () {{
  ...
  rows.forEach(function (r) {{
    r.dataset.text = r.textContent.toLowerCase();
    r.dataset.verified = r.querySelector('.badge') ? '1' : '0';
    r.dataset.icn = /t-L/.test(r.className) ? '1' : '0';
    var closes = (r.cells[3] ? r.cells[3].textContent : '');
    var d = Date.parse(closes.replace(/[/]/g, '-'));
    var days = isNaN(d) ? 9999 : (d - Date.now()) / 86400000;
    r.dataset.soon = (days >= 0 && days <= 14) ? '1' : '0';
  }});

  function apply() {{
    var term = q.value.trim().toLowerCase(), shown = 0;
    rows.forEach(function (r) {{
      var ok = (!term || r.dataset.text.indexOf(term) !== -1)
        && (mode === 'all'
            || (mode === 'verified' && r.dataset.verified === '1')
            || (mode === 'soon'     && r.dataset.soon     === '1')
            || (mode === 'icn'      && r.dataset.icn      === '1'));
